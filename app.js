...
const validator = require("express-validator"); // validating input
const csurf = require("csurf") // CSRF tokens to prevent attacks
const cookieParser = require("cookie-parser"); // to prevent user cookie manipulation
...
app.get("/transfer", csrfMiddleware, function (request, response) {
  if (request.session.loggedin) {
    var sent = "";
    response.render("transfer", { sent, crsfToken: request.csrfToken() });
  } else {
    response.redirect("/");
  }
});
...
app.post("/download", function (request, response) {
  if (request.session.loggedin) {
    ...
    const filePath = path.join(process.cwd() + '/history_files/', file_name); // improves security by not hardcoding the file path
...
//SQL UNION INJECTION VULNERABILITY
